---
layout: default
title: 关于我们
lang: zh
permalink: /zh/company/about/
description: 关于 Spirit Connect AIPE Labs —— 连接 AI 辅助工程与电力基础设施。
---

<header class="hero hero-compact">
  <div class="container">
    <h1>关于 Spirit Connect AIPE Labs</h1>
    <p class="lead">
      我们正在构建贯穿器件、变换器与系统的智能设计方法，
      形成 AI 与能源基础设施共同演进的闭环生态。
    </p>
  </div>
</header>

<section class="section">
  <div class="container">
    <h2>我们的使命</h2>
    <p>
      AIPE 最初代表 <strong>AI for Power Electronics（人工智能赋能电力电子）</strong>，体现了我们将 AI
      应用于电力电子设计的最初方向。随着工作从器件和变换器拓展到完整的能源系统，AIPE 逐步演进为
      <strong>AI for Power Engineering（人工智能赋能电力工程）</strong>，在原有基础上形成了更宽广的愿景。
    </p>
    <p>
      这一成长过程没有改变我们的核心使命：构建一个闭环生态，让 AI 改进电力与能源基础设施的设计，
      同时让工程数据和真实应用经验持续提升 AI 的能力。
    </p>
  </div>
</section>

<section class="section section-alt">
  <div class="container">
    <h2>闭环生态</h2>
    <p class="lead section-lead-spaced">
      我们设想一个自我强化的循环，同时加速 AI 能力与能源基础设施的发展。
    </p>
    <div class="grid">
      <div class="card">
        <h3>AI → 电力</h3>
        <p>AI 智能体帮助工程师设计更优的电力变换器与能源系统 —— 缩短开发周期、提升效率，
        并探索人力难以覆盖的设计空间。</p>
      </div>
      <div class="card">
        <h3>电力 → AI</h3>
        <p>由此产生的电力基础设施 —— 高功率密度变换器、高效微电网、直流配电网络 ——
        为 AI 数据中心与算力集群提供清洁、可靠的能源。</p>
      </div>
      <div class="card">
        <h3>反馈 → 演进</h3>
        <p>运行数据、验证结果与真实性能反馈给 AI 智能体。它不断完善模型、从每次部署中学习，
        并在下一轮迭代中产出更优的设计。</p>
      </div>
    </div>
  </div>
</section>

<section class="section">
  <div class="container">
    <h2>三个设计尺度</h2>
    <p>我们的 AI 智能体并非只在单一层级工作 —— 它贯穿完整的电力电子层级进行推理。</p>
    <div class="grid">
      <div class="card">
        <h3>器件</h3>
        <p>SiC、GaN 与 Si 半导体表征、损耗建模、热特性分析，以及从丰富的元件数据库中自动选型。</p>
      </div>
      <div class="card">
        <h3>变换器</h3>
        <p>面向 Buck/Boost、LLC、DAB、多电平逆变器等的拓扑探索、磁性元件设计、控制综合与多目标优化。</p>
      </div>
      <div class="card">
        <h3>系统</h3>
        <p>直流与交流微电网、混合配电架构，以及电网级协同优化 —— 让每一台变换器都服务于整个系统的任务需求。</p>
      </div>
    </div>
  </div>
</section>

<section class="section section-alt partners-section">
  <div class="container">
    <h2>合作伙伴</h2>
    <p>连接人工智能、电力电子与能源系统研究的学术和产业合作关系。</p>
    {% include partners-marquee.html %}
  </div>
</section>

<section class="section">
  <div class="container">
    <h2>联系我们</h2>
    <p>
      对 AI 辅助电力电子工程感兴趣，或希望探讨合作？
      欢迎发邮件至 <a href="mailto:info@spiritconnect.co.uk">info@spiritconnect.co.uk</a>，或在
      <a href="https://www.linkedin.com/in/fulong-li-6bb443127" target="_blank">领英</a> 上与我们联系。
    </p>
  </div>
</section>
